import { DummyDataGenerators, DummyDataConfig } from '../types/import-export.js';
...
export class DummyDataGenerator implements DummyDataGenerators {
    ...
    if (method === 'GET') {
      if (url.includes('/users') || url.includes('/user')) {
        if (!params.some(p => p.key === 'page')) {
          params.push({ key: 'page', value: '1', description: 'Page number for pagination' });
        }
        if (!params.some(p => p.key === 'limit')) {
          params.push({ key: 'limit', value: '10', description: 'Number of items per page' });
        }
        if (!params.some(p => p.key === 'sort')) {
          params.push({ key: 'sort', value: 'created_at', description: 'Sort field' });
        }
      }

      if (url.includes('/search')) {
        if (!params.some(p => p.key === 'q')) {
          params.push({ key: 'q', value: 'example search', description: 'Search query' });
        }
      }

      if (url.includes('/filter')) {
        if (!params.some(p => p.key === 'status')) {
          params.push({ key: 'status', value: 'active', description: 'Filter by status' });
        }
      }

      // Add format parameter for APIs
      if (!params.some(p => p.key === 'format')) {
        params.push({ key: 'format', value: 'json', description: 'Response format' });
      }
    }
  ...
  headers(method: string, hasBody: boolean = false): Array<{ key: string; value: string; description?: string }> {
  ...
  private generateValueForParam(paramName: string): string {
    ...
    // Boolean
    if (lowerParam.includes('active') || lowerParam.includes('enabled')) return 'true';
  ...
  private generateParamDescription(paramName: string): string {
    ...
    if (lowerParam === 'limit' || lowerParam === 'size') return 'Number of items to return';
    if (lowerParam === 'offset') return 'Number of items to skip';
    if (lowerParam.includes('search') || lowerParam === 'q') return 'Search query string';
  ...
  private generateFormData(isMultipart: boolean = false): Array<{ key: string; value: string; type?: string; description?: string }> {
    const formData: Array<{ key: string; value: string; type?: string; description?: string }> = [
      { key: 'name', value: 'Sample Name', description: 'Item name' },
      { key: 'description', value: 'Sample description', description: 'Item description' },
...
<request>
  ...
  private generateEnvironmentValue(varName: string): string {
    ...
    if (lowerVar.includes('url') || lowerVar.includes('host')) return 'https://api.example.com';
    if (lowerVar.includes('token') || lowerVar.includes('key')) return 'your_api_key_here';
  ...
  private isSecretVariable(varName: string): boolean {
    const lowerVar = varName.toLowerCase();
    return lowerVar.includes('token') || 
  ...
  private generateEnvironmentDescription(varName: string): string {
    ...
    if (lowerVar.includes('url') || lowerVar.includes('host')) return 'Base URL for the API';
