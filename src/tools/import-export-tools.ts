...
const ExportCollectionSchema = z.object({
  ...
  includeDummyData: z
    ...
    .optional()
    .default(true)
    .describe('Include generated dummy data'),
  generateEnvironmentTemplate: z
    .boolean()
    .optional()
    .default(true)
    .describe('Generate environment template'),
  outputPath: z
    .string()
    .optional()
    .describe('Local file path to save the export (optional)'),
});
...
const ExportWorkspaceCollectionsSchema = z.object({
  workspaceId: z
    .string()
    .describe('The ID of the workspace to export collections from'),
  format: z
    .enum(['postman', 'insomnia', 'openapi'])
    .optional()
    .default('postman')
  ...
  outputDirectory: z
    .string()
    .optional()
    .describe('Local directory to save exports (optional)'),
});
...
const ValidateCollectionExportSchema = z.object({
  ...
  format: z
    ...
    .optional()
    .default('postman')
    .describe('Format to validate against'),
});

const ImportCollectionSchema = z.object({
  collectionData: z
    .string()
    .describe('JSON string of the collection data to import'),
  targetWorkspaceId: z
    .string()
    .optional()
    .describe('Target workspace ID (optional)'),
  conflictResolution: z
    .enum(['skip', 'overwrite', 'rename'])
    .optional()
    .default('rename')
    .describe('How to handle conflicts'),
  validateBeforeImport: z
    .boolean()
    .optional()
    .default(true)
    .describe('Validate before importing'),
});
...
export const importExportTools: Tool[] = [
  {
    ...
    description:
      'Export a Postman collection to various formats with optional dummy data generation',
    inputSchema: {
      type: 'object',
      properties: {
        collectionId: {
  ...
  },
  {
    ...
    description:
      'Export a collection with comprehensive dummy data for immediate use in Postman/Insomnia',
    inputSchema: {
      type: 'object',
      properties: {
        collectionId: {
  ...
  },
  {
    ...
    description:
      'Validate a collection export for compatibility with target format',
    inputSchema: {
      type: 'object',
      properties: {
        collectionId: {
...
export async function handleImportExportTool(
  ...
  switch (name) {
    ...
    case 'export_workspace_collections': {
      ...
      const result = await exportWorkspaceCollections(
        client,
        formatConverter,
        dummyDataGenerator,
        parsed
    ...
    case 'validate_collection_export': {
      ...
      const result = await validateCollectionExport(
        client,
        formatConverter,
        parsed
      );
...
async function exportCollection(
  ...
  try {
    ...
    switch (options.format) {
      case 'postman':
        exportData = formatConverter.toPostmanV21(
          collection,
          options.includeDummyData
        );
        break;
      case 'insomnia':
        exportData = formatConverter.toInsomniaV4(
          collection,
          options.includeDummyData
        );
        break;
    ...
    if (options.generateEnvironmentTemplate && options.includeDummyData) {
      const envVars =
        dummyDataGenerator.generateEnvironmentVariables(collection);
      environmentData = {
    ...
    if (options.outputPath) {
      ...
      if (environmentData) {
        const envPath = options.outputPath.replace(
          /\.[^/.]+$/,
          '.environment.json'
        );
      ...
      const exportOptions = {
        ...
        outputPath: options.outputDirectory
          ? path.join(
              ...
              `${collectionSummary.name}.${options.format}.json`
            )
          : undefined,
      };
...
async function validateCollectionExport(
  ...
  try {
    ...
    try {
      ...
      errors.push(
        `Failed to convert to ${options.format}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
...
async function importCollection(
  ...
  try {
    ...
    const collectionData = JSON.parse(options.collectionData) as {
